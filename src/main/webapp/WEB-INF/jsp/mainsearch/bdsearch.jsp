...
<title>목록</title>
   ...
   <!-- 게시판 -->
   		<table class="table" style="text-align: center; width: 67%; margin: auto;">
			...
			<c:forEach var="u" items="${bdlist}" varStatus="status" >
				<tr>
					<td>${u.num}</td>
...
<div>
  <ul class="pagination">
     <li class="page-item">
        <c:if test="${bdpage.pages>5}">
      <a class="page-link" href="/petmong/bdsearch?uid=${user.user.uid}&keyword=${keyword}&pageNum=${bdpage.navigateFirstPage-1}">&laquo;</a>
        </c:if>
    </li>
  
    <c:forEach begin="${bdpage.navigateFirstPage}" end="${bdpage.navigateLastPage}" var="num">
<c:choose>
    <c:when test="${bdpage.pageNum==num}">
    <li class="page-item active">
      <a class="page-link" href="/petmong/bdsearch?uid=${user.user.uid}&keyword=${keyword}&pageNum=${num}">${num}</a>
    </li>
    </c:when>
    <c:otherwise>
    <li class="page-item">
     ...
     <li class="page-item">
        <c:if test="${bdpage.pages>5}">
      <a class="page-link" href="/petmong/bdsearch?uid=${user.user.uid}&keyword=${keyword}&pageNum=${bdpage.navigateLastPage+1}">&raquo;</a>
        </c:if>
    </li>
  </ul>
</div>
